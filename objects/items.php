...
class items
	...
	function readJasa($from_record_num, $records_per_page){	 
		    $query = "SELECT * FROM " . $this->table_name . "  WHERE item_type='JASA' ORDER BY item_kode ASC LIMIT {$from_record_num}, {$records_per_page}";	 
		    $statement = $this->connect->prepare( $query );
		    $statement->execute();	 
		    return $statement;
		}
	...
	function ShowBarang2($from_record_num, $records_per_page){
			...
			"FROM tbstok INNER JOIN tbitem ".
			"ON (tbstok.item_kode = tbitem.item_kode) " .
