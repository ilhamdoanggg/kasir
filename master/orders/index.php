...
echo "				</div>";
	...
	$orders->cust_kode = !empty ($_SESSION['current_customer']) ? $_SESSION['current_customer'] : '-';
	$statement = $orderdetail->ShowTemp($orders->order_id);
	$bayar = isset($bayar) ? $bayar : 0;
	$kembali = isset($kembali) ? $kembali : 0;

	$_SESSION['bayar'] = null; //!empty($_SESSION['bayar']) ? $_SESSION['bayar'] : null;
	$_SESSION['kembali'] = null; //!empty($_SESSION['kembali']) ? $_SESSION['kembali'] : null;
...
$(document).on('click', '#btnproses', function(){
	...
	var bayar = accounting.unformat(document.getElementById("txtbayar").value);
	var total = document.getElementById("hidden-total").value;
	var kembalian = bayar - total;
